// Based on JPEG Writer from http://www.jonolick.com/code.html
// Original copyright:
...
static void jo_writeBits(QDataStream &stream, int &bitBuf, int &bitCnt, const unsigned short *bs) {
    bitCnt += bs[1];
    bitBuf |= bs[0] << (24 - bitCnt);
    while(bitCnt >= 8) {
        unsigned char c = (bitBuf >> 16) & 255;
        stream.writeRawData((const char*)(&c), 1);
        if(c == 255) {
            stream << (quint8)0;
        }
        bitBuf <<= 8;
...
static int jo_processDU(QDataStream &stream, int &bitBuf, int &bitCnt, float *CDU, float *fdtbl, int DC, const unsigned short HTDC[256][2], const unsigned short HTAC[256][2]) {
    ...
    int diff = DU[0] - DC;
    if (diff == 0) {
        jo_writeBits(stream, bitBuf, bitCnt, HTDC[0]);
    } else {
        unsigned short bits[2];
        jo_calcBits(diff, bits);
        jo_writeBits(stream, bitBuf, bitCnt, HTDC[bits[1]]);
        jo_writeBits(stream, bitBuf, bitCnt, bits);
    }
    // Encode ACs
    int end0pos = 63;
    ...
    if(end0pos == 0) {
        jo_writeBits(stream, bitBuf, bitCnt, EOB);
        return DU[0];
    }
    for(int i = 1; i <= end0pos; ++i) {
        ...
        if ( nrzeroes >= 16 ) {
            ...
            for (int nrmarker=1; nrmarker <= lng; ++nrmarker)
                jo_writeBits(stream, bitBuf, bitCnt, M16zeroes);
            nrzeroes &= 15;
        }
        unsigned short bits[2];
        jo_calcBits(DU[i], bits);
        jo_writeBits(stream, bitBuf, bitCnt, HTAC[(nrzeroes<<4)+bits[1]]);
        jo_writeBits(stream, bitBuf, bitCnt, bits);
    }
    if(end0pos != 63) {
        jo_writeBits(stream, bitBuf, bitCnt, EOB);
...
void jo_write_jpg(QDataStream &stream, const QImage &image, int quality = 0) {
    // Convert QImage to rgb
    int width = image.width();
    int height = image.height();
    int comp = 3;
    unsigned char *data = (unsigned char *)malloc(width * height * comp);
    for(int y = 0; y < height; y++) {
        for(int x = 0; x < width; x++) {
            unsigned offset = comp * (y * width + x);
            QRgb pixel = image.pixel(x, y);
            data[offset]   = qRed(pixel);
            data[offset+1] = qGreen(pixel);
            data[offset+2] = qBlue(pixel);
        }
    }
    ...
    stream.writeRawData((const char*)(head0), sizeof(head0));
    stream.writeRawData((const char*)(YTable), sizeof(YTable));
    stream << (quint8)1;
    stream.writeRawData((const char*)(UVTable), sizeof(UVTable));
    ...
    stream.writeRawData((const char*)(head1), sizeof(head1));
    stream.writeRawData((const char*)(std_dc_luminance_nrcodes+1), sizeof(std_dc_luminance_nrcodes)-1);
    stream.writeRawData((const char*)(std_dc_luminance_values), sizeof(std_dc_luminance_values));
    stream << (quint8)0x10; // HTYACinfo
    stream.writeRawData((const char*)(std_ac_luminance_nrcodes+1), sizeof(std_ac_luminance_nrcodes)-1);
    stream.writeRawData((const char*)(std_ac_luminance_values), sizeof(std_ac_luminance_values));
    stream << (quint8)1; // HTUDCinfo
    stream.writeRawData((const char*)(std_dc_chrominance_nrcodes+1), sizeof(std_dc_chrominance_nrcodes)-1);
    stream.writeRawData((const char*)(std_dc_chrominance_values), sizeof(std_dc_chrominance_values));
    stream << (quint8)0x11; // HTUACinfo
    stream.writeRawData((const char*)(std_ac_chrominance_nrcodes+1), sizeof(std_ac_chrominance_nrcodes)-1);
    stream.writeRawData((const char*)(std_ac_chrominance_values), sizeof(std_ac_chrominance_values));
    static const unsigned char head2[] = { 0xFF,0xDA,0,0xC,3,1,0,2,0x11,3,0x11,0,0x3F,0 };
    stream.writeRawData((const char*)(head2), sizeof(head2));
    ...
    for(int y = 0; y < height; y += 8) {
        for(int x = 0; x < width; x += 8) {
            ...
            DCY = jo_processDU(stream, bitBuf, bitCnt, YDU, fdtbl_Y, DCY, YDC_HT, YAC_HT);
            DCU = jo_processDU(stream, bitBuf, bitCnt, UDU, fdtbl_UV, DCU, UVDC_HT, UVAC_HT);
            DCV = jo_processDU(stream, bitBuf, bitCnt, VDU, fdtbl_UV, DCV, UVDC_HT, UVAC_HT);
        }
    }

    // Do the bit alignment of the EOI marker
    static const unsigned short fillBits[] = {0x7F, 7};
    jo_writeBits(stream, bitBuf, bitCnt, fillBits);
